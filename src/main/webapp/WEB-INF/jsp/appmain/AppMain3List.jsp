						...
						<div class="pull-right" style="padding-top: 10px">
							<ul class="list-unstyled">
								<li style="float: left;"><input type="checkbox"
									name="searchType" value="text2"
									<c:if test="${fn:indexOf(searchVO.searchType, 'text2')!= -1}">checked="checked"</c:if> />
									<label class="chkselect" for="searchType1">해시태그</label>&nbsp;&nbsp;

									<input type="checkbox" name="searchType" value="link"
									<c:if test="${fn:indexOf(searchVO.searchType, 'link')!=-1}">checked="checked"</c:if> />
									<label class="chkselect" for="searchType2">YTB링크</label>&nbsp;&nbsp;
								</li>
									...
									<tbody>
										<c:forEach var="listview" items="${listview}"
											...
											<tr>
												...
												<td><a href="/admin/appmain3Read?sn=${listview.sn}">${listview.link}</a></td>
...
<script>
	function fn_formGo() {
		location.href = "/admin/appmain3Form";
	}
</script>
...
<script>
	function fn_readGo(_a) {
		...
		document.form1.action = "appmain3Read";
		document.form1.submit();
	}
</script>
